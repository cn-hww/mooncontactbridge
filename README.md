...
解析器要求每张卡包含 `VERSION:4.0` 和 `FN`。参数和未知的 `X-` 字段会原样进入模型，便于后续检查。联系人配对只接受迁移前后都唯一的 UID、邮箱或号码；共享标识会跳过，若没有其他唯一标识，联系人就保留为未匹配项。
...
参数比较遵循内容行的语义：属性名和参数名不区分大小写，属性组编号与参数顺序不影响结果，`TYPE` 的多个值也不受大小写和排列顺序影响。
